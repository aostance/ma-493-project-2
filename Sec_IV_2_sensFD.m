%%% MA 493 - Topic IV.2 
%%% Compute scaled sensitivites based on forward finite difference 
%%% approximations
...
s1 = 2*alpha*mNom*(fOD(t,thetaNom+[h,0,0,0])-y)/h;
s2 = 2*alpha*kNom*(fOD(t,thetaNom+[0,h,0,0])-y)/h;
s3 = 2*alpha*cNom*(fOD(t,thetaNom+[0,0,h,0])-y)/h;
s4 = 2*alpha*ANom*(fOD(t,thetaNom+[0,0,0,h])-y)/h;
...
yData = fOD(tData,thetaNom);
S(:,1) = 2*alpha*mNom*(fOD(tData,thetaNom+[h,0,0,0])-yData)/h;
S(:,2) = 2*alpha*kNom*(fOD(tData,thetaNom+[0,h,0,0])-yData)/h;
S(:,3) = 2*alpha*cNom*(fOD(tData,thetaNom+[0,0,h,0])-yData)/h;
S(:,4) = 2*alpha*ANom*(fOD(tData,thetaNom+[0,0,0,h])-yData)/h;
